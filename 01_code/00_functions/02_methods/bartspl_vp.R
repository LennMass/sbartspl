bartspl_vp<-function(datall,RO,
									...
									n_forest=4,
									var_infl = 0, # just for "sbart", "dart", "smooth_bart: no variance inflation
									var_infl_param = 10, # for dbarts: default in Nethery et al. (2019) -> 10
	...
	for (i in 1:(nburn+nsamp)){
		...
		if (nrow(datno1)>0){
			...
			if(softbart_sampler==FALSE){      
				delta_star[which(datno$x==1)]<-rnorm(n=nrow(datno1),mean=Eppd,sd=sqrt(sigma_spl1+ROdist[which(datno$x==1)]*var_infl_param*(max(delta)-min(delta))))
			}
		...
		if (nrow(datno0)>0){
			...
			if(softbart_sampler==FALSE){
				delta_star[which(datno$x==0)]<-rnorm(n=nrow(datno0),mean=Eppd,sd=sqrt(sigma_spl0+ROdist[which(datno$x==0)]*var_infl_param*(max(delta)-min(delta))))
			}
			if(softbart_sampler==TRUE){
