...
# read in dataset
test2 <- readRDS("00_data/02_application/02_preprocess/leukemia.rds")

## identify the RO and RN ##
RO<-pw_overlap(ps=test2$ps,E=test2$expose,a=.1*(max(test2$ps)-min(test2$ps)),b=10)

####
# Estimate effect of exposure on leukemia mortality rates ----
####
...
saveRDS(results_tab, "02_results/02_application/results_leukemia.rds")
...
####
# outcome: CHANGE in leukemia mortality rate ----
####

set.seed(4)

## create an empty table to store change in leukemia analysis results ##
results_tab<-NULL
...
saveRDS(results_tab, "02_results/02_application/results_leukemia_change.rds")
